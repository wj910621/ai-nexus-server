...
class AgentEngine {
  ...
  async _callLLM(messages, model) {
    const key = this._getApiKey(model); if (!key) return { content: 'No API Key', role: 'assistant' };
    try { const r = await fetch('https://www.dmxapi.cn/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key }, body: JSON.stringify({ model, messages, temperature: 0.7, max_tokens: 4096, stream: false }), signal: AbortSignal.timeout(60000) }); if (!r.ok) return { content: 'API Error: ' + r.status, role: 'assistant' }; const d = await r.json(); return { content: (d.choices && d.choices[0] && d.choices[0].message && d.choices[0].message.content) || '', role: 'assistant' }; } catch(e) { return { content: 'Error: ' + e.message, role: 'assistant' }; }
  }
  async run(task, options) {
    ...
    const msgs = [{ role: 'system', content: '����TriGen AI Agent�����ù���: ' + toolNames + '����Ҫ����ʱ������ж���������\n���룺{"����":"ֵ"}\n���ʱ��������ջش���Ĵ�' }, { role: 'user', content: task }];
    ...
    while (iter < maxIter) {
      ...
      if (/���ջش�/.test(content)) { var m = content.match(/(?:���ջش�)[��:]\s*([\s\S]*)/); answer = m ? m[1].trim() : content; if (streamCb) streamCb({ type: 'final', content: answer }); break; }
      var actMatch = content.match(/�ж�[��:]\s*(\w+)/); var inpMatch = content.match(/����[��:]\s*(\{[\s\S]*?\})/);
      if (actMatch) {
        if (streamCb) streamCb({ type: 'thought', content, action: actMatch[1] });
        try { var input = inpMatch ? JSON.parse(inpMatch[1]) : {}; var result = await tools.execute(actMatch[1], input, { workspace }); var obs = JSON.stringify(result).substring(0, 2000); msgs.push({ role: 'assistant', content }); msgs.push({ role: 'user', content: '�۲죺' + obs + '\n\n���������"���ջش�"' }); history.push({ action: actMatch[1], input, result }); if (streamCb) streamCb({ type: 'observation', action: actMatch[1], result }); } catch(e) { msgs.push({ role: 'assistant', content }); msgs.push({ role: 'user', content: '���ߴ���' + e.message }); }
