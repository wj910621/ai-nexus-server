...
# Create the new download page with dynamic model count
...
.model-count{font-size:12px;color:#a090c0;margin-top:8px;opacity:0.8}
</style>
</head>
<body>
<div class="card">
  ...
  <h1>TriGenClaw</h1>
  <p>AI 大模型桌面客户端<br><span id="modelCountDisplay">加载中...</span></p>
...
(async function(){
  try{
    const r=await fetch(window.location.origin+'/api/models-count',{signal:AbortSignal.timeout(5000)});
    if(r.ok){const d=await r.json();document.getElementById('modelCountDisplay').textContent=d.count+' 个大模型一键访问，写作/聊天/编程/绘图全能'}
    else document.getElementById('modelCountDisplay').textContent='135+ 个大模型一键访问'
  }catch(e){
    document.getElementById('modelCountDisplay').textContent='135+ 个大模型一键访问'
  }
})();
</script>
</body>
...
# Now also upload the updated index.html
sftp = ssh.open_sftp()
print("Uploading updated index.html with TriGenClaw + 开通会员...")
...
print("Nginx reloaded:", stdout.read().decode().strip()[:100])
